...
osStatus_t SvVis3_send_message(SvVis3_t *tar, SvVis3_message_t *message)
{
    while(osMessageQueueGetSpace(tar->queue_send) < 1) {osThreadYield();} // wait for available space in send queue
    return osMessageQueuePut(tar->queue_send, message, NULL, osWaitForever);
}

osStatus_t SvVis3_send_string(SvVis3_t *tar, const char *str)
{
    ...
    return SvVis3_send_message(tar, &message);
}
osStatus_t SvVis3_send_I16(SvVis3_t *tar, SvVis3_channel_t channel, int16_t data)
{
...
osStatus_t SvVis3_send_I32(SvVis3_t *tar, SvVis3_channel_t channel, int32_t data)
{
    size_t len = sizeof(data);
...
osStatus_t SvVis3_send_float(SvVis3_t *tar, SvVis3_channel_t channel, float data)
{
    size_t len = sizeof(data);
